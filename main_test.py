...
class ReviewBot:
    ...
    def run(self):
        ...
        try:
            ...
            logger.info("=" * 70)
            logger.info(f"📍 Store: {settings.STORE_URL}")
            add_web_log('info', f'Store: {settings.STORE_URL}')
            ...
            with BrowserManager() as page:
                ...
                add_web_log('info', 'Scanning homepage for collections...')
                collections = scraper.get_homepage_collections(settings.STORE_URL)
                        ...
                        for prod_idx, product_url in enumerate(products, 1):
                            ...
                            try:
                                ...
                                for review_num in range(1, settings.REVIEWS_PER_PRODUCT + 1):
                                    ...
                                    logger.info(f"👤 Name: {review_data['name']}")
                                    logger.info(f"📧 Email: {review_data['email']}")
                                    logger.info(f"💬 Review: {review_data['text'][:80]}...")
                                    
                                    # Process image (optional)
                                    image_path = None
                                    if settings.USE_AI_IMAGES and product_data['image_url']:
                                        image_path = image_processor.download_and_process(
                                            product_data['image_url'],
                                            product_data['name']
                                        )
                                        if image_path:
                                            self.stats['images_generated'] += 1
                                            add_web_log('success', 'Processed review image')
                                    
                                    # Post review to Loox
                                    success = poster.post_review(review_data, image_path)
                                    ...
                                    # Cleanup image
                                    if image_path:
                                        image_processor.cleanup(image_path)
    ...
    def _print_stats(self):
        ...
        add_web_log('info', f"Collections: {self.stats['collections_processed']}, Products: {self.stats['products_processed']}, Reviews Posted: {self.stats['reviews_posted']}")
